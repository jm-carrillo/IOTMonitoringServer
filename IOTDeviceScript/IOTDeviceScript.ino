...
#define LED_PR 2 // LED VENTILADOR
#define LED_PR_2 0 // LED AIRE ACONDICIONADO
String alerta_1 = "";
String alerta_2 = "";
...
const char ssid[] = "TMOBILE-5F41"; // TODO cambiar por el nombre de la red WiFi
// Contraseña de la red WiFi
const char pass[] = "Colombia.2015"; // TODO cambiar por la contraseña de la red WiFi
...
const char MQTT_HOST[] = "44.211.59.27"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
...
void receivedCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (data.indexOf("ALERT") >= 0) {  
    alert = data;
    Serial.println(data);  
    if (data.indexOf("Ventilador") >= 0){
      digitalWrite(LED_PR, HIGH);
      alerta_1 = "Ventilador";      

    } else if(data.indexOf("Aire Acondicionado") >= 0){
      digitalWrite(LED_PR_2, HIGH);
      Serial.println("Aire Acondicionado");      
    }
  } else {
    digitalWrite(LED_PR, LOW);
    digitalWrite(LED_PR_2, LOW);
  } 
}
...
void setup() {
  ...
  pinMode(LED_PR_2, OUTPUT);
  digitalWrite(LED_PR, LOW); //LED comienza apagado
  digitalWrite(LED_PR_2, LOW); //LED comienza apagado
